...
class BaseDaemon:
    ...
    def parse_xpub(self, xpub):
        ...
        if isinstance(xpub, dict):
            return xpub.get("xpub", None), xpub.get("contracts", [])
    ...
    async def get_handle_request_params(self, request):
        ...
        xpub, contracts = self.parse_xpub(kwargs.pop("xpub", None))
        return id, method, xpub, contracts, args, kwargs, error

    @authenticate
    async def handle_request(self, request):
        id, req_method, xpub, contracts, req_args, req_kwargs, error = await self.get_handle_request_params(request)
        if error:
            return error.send()
        return await self.execute_method(id, req_method, xpub, contracts, req_args, req_kwargs)
    ...
    async def execute_method(self, id, req_method, xpub, contracts, req_args, req_kwargs):
        ...
        Args:
            ...
            contracts (list): list of smart contract addresses
