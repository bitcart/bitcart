...
def _worker_health_matcher(name: str, attributes: "Attributes | None") -> bool:
    ...
    return lower_name.startswith("recording health:") or lower_name.startswith("health check successful")


def configure_logfire(settings: Settings, service_name: Literal["server", "worker"]) -> None:
...
def instrument_fastapi(settings: Settings, app: FastAPI) -> None:
    if settings.is_testing():
        return
...
def instrument_sqlalchemy(settings: Settings, engine: Engine) -> None:
    if settings.is_testing():
        return
