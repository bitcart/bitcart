...
async def run_tests():  # noqa: C901
    ...
    try:
        ...
        check_result("XRPLRPCProvider created", True)
    except Exception as e:
        check_result("XRPLRPCProvider created", False, str(e))
    ...
    try:
        ...
        check_result("send_ping_request (AbstractRPCProvider)", True)
    except Exception as e:
        check_result("send_ping_request (AbstractRPCProvider)", False, str(e))
    ...
    try:
        ...
        check_result("send_single_request (AbstractRPCProvider)", resp.is_successful())
    except Exception as e:
        check_result("send_single_request (AbstractRPCProvider)", False, str(e))
    ...
    try:
        ...
        check_result("MultipleProviderRPC.send_request", resp.is_successful())
    except Exception as e:
        check_result("MultipleProviderRPC.send_request", False, str(e))
    ...
    try:
        ...
        check_result("is_connected", connected)
    except Exception as e:
        check_result("is_connected", False, str(e))

    # get_block_number
    ledger_index = None
    try:
        ledger_index = await coin.get_block_number()
        check_result("get_block_number", isinstance(ledger_index, int) and ledger_index > 0, f"ledger_index={ledger_index}")
    except Exception as e:
        check_result("get_block_number", False, str(e))

    # get_gas_price (fee)
    try:
        fee = await coin.get_gas_price()
        check_result("get_gas_price (fee in drops)", isinstance(fee, int) and fee > 0, f"fee={fee} drops")
    except Exception as e:
        check_result("get_gas_price", False, str(e))

    # get_balance
    try:
        balance = await coin.get_balance(KNOWN_TESTNET_ADDRESS)
        check_result("get_balance (genesis)", isinstance(balance, Decimal), f"balance={balance} drops")
    except Exception as e:
        check_result("get_balance", False, str(e))
    ...
    try:
        ...
        check_result("get_balance (non-existent = 0)", zero_bal == Decimal(0))
    except Exception as e:
        check_result("get_balance (non-existent)", False, str(e))

    # get_block
    try:
        block = await coin.get_block("validated")
        check_result(
            "get_block('validated')", isinstance(block, dict) and "close_time" in block, f"keys={list(block.keys())[:5]}"
        )
    except Exception as e:
        check_result("get_block", False, str(e))

    # get_block by number
    if ledger_index:
        try:
            block = await coin.get_block(ledger_index)
            check_result("get_block(number)", isinstance(block, dict) and "close_time" in block)
        except Exception as e:
            check_result("get_block(number)", False, str(e))

    # get_block_txes
    if ledger_index:
        try:
            txes = await coin.get_block_txes(ledger_index)
            check_result("get_block_txes", isinstance(txes, list), f"count={len(txes)}")
        except Exception as e:
            check_result("get_block_txes", False, str(e))

    # chain_id
    try:
        cid = await coin.chain_id()
        check_result("chain_id", cid == 0)
    except Exception as e:
        check_result("chain_id", False, str(e))

    # ── 3. Address Validation ──
    print("\n--- Address Validation ---")

    check_result("is_address (valid classic)", coin.is_address("rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh"))
    check_result("is_address (invalid)", not coin.is_address("not_an_address"))

    # X-address
    from xrpl.core.addresscodec import classic_address_to_xaddress

    x_addr = classic_address_to_xaddress(KNOWN_TESTNET_ADDRESS, None, True)
    check_result("is_address (valid X-address)", coin.is_address(x_addr))

    # normalize_address
    normalized = coin.normalize_address(x_addr)
    check_result("normalize_address (X-addr to classic)", normalized == KNOWN_TESTNET_ADDRESS, f"got={normalized}")
    check_result(
        "normalize_address (classic passthrough)", coin.normalize_address(KNOWN_TESTNET_ADDRESS) == KNOWN_TESTNET_ADDRESS
    )
    ...
    try:
        ...
        check_result("get_payment_uri with dest_tag", "dt=12345" in uri and KNOWN_TESTNET_ADDRESS in uri, f"uri={uri}")
    except Exception as e:
        check_result("get_payment_uri", False, str(e))

    try:
        uri_no_tag = await coin.get_payment_uri(KNOWN_TESTNET_ADDRESS, Decimal("1"), 6)
        check_result("get_payment_uri without dest_tag", "dt=" not in uri_no_tag, f"uri={uri_no_tag}")
    except Exception as e:
        check_result("get_payment_uri (no tag)", False, str(e))
    ...
    try:
        ...
        check_result(
            "KeyStore.add_privkey", ks_reimport.private_key == ks.private_key and ks_reimport.public_key == ks.public_key
        )
    except Exception as e:
        check_result("KeyStore.add_privkey", False, str(e))

    # add_privkey mismatch
    try:
        ks_mismatch = KeyStore(key=KNOWN_TESTNET_ADDRESS)
        ks_mismatch.add_privkey(seed)
        check_result("KeyStore.add_privkey rejects mismatch", False, "Should have raised")
    except Exception as e:
        check_result("KeyStore.add_privkey rejects mismatch", "mismatch" in str(e).lower())
    ...
    try:
        ...
        check_result(
            ...
            tx is not None
            and tx.hash == "ABC123"
            and tx.destination_tag == 42
            and tx.value == 1000000
            and tx.to == KNOWN_TESTNET_ADDRESS,
        )
    except Exception as e:
        check_result("process_tx_data (valid payment)", False, str(e))

    # process_tx_data — non-Payment type
    try:
        tx_offer = {"TransactionType": "OfferCreate", "hash": "X"}
        result = await coin.process_tx_data(tx_offer)
        check_result("process_tx_data (non-Payment  -> None)", result is None)
    except Exception as e:
        check_result("process_tx_data (non-Payment)", False, str(e))
    ...
    try:
        ...
        check_result("process_tx_data (failed tx  -> None)", result is None)
    except Exception as e:
        check_result("process_tx_data (failed tx)", False, str(e))
    ...
    try:
        ...
        check_result("process_tx_data (issued currency  -> None)", result is None)
    except Exception as e:
        check_result("process_tx_data (issued currency)", False, str(e))
    ...
    try:
        ...
        check_result(
            "process_tx_data (partial payment protection)",
            tx is not None and tx.value == 3000000,
            f"value={tx.value if tx else 'None'} (expected 3000000)",
        )
    except Exception as e:
        check_result("process_tx_data (partial payment)", False, str(e))
    ...
    try:
        ...
        check_result(
            "Invoice dataclass creation",
            inv.destination_tag == 42 and inv.payment_address == "42" and inv.address == KNOWN_TESTNET_ADDRESS,
        )
    except Exception as e:
        check_result("Invoice dataclass creation", False, str(e))

    # Invoice inherits BaseInvoice fields
    try:
        check_result(
            "Invoice has BaseInvoice fields",
            hasattr(inv, "status") and hasattr(inv, "sent_amount") and hasattr(inv, "payment_address"),
        )
    except Exception as e:
        check_result("Invoice inheritance", False, str(e))
    ...
    try:
        ...
        for _ in range(100):
            ...
            tags.add(tag)
        check_result("generate_destination_tag (100 unique tags)", len(tags) == 100)
    except Exception as e:
        check_result("generate_destination_tag", False, str(e))
    ...
    try:
        with open(spec_path) as f:  # noqa: ASYNC230
            spec = json.load(f)
        check_result("xrp.json loads", isinstance(spec, dict))
        check_result("xrp.json has exceptions", "exceptions" in spec and isinstance(spec["exceptions"], dict))
        # Check a few expected error codes
        has_errors = all(k in spec["exceptions"] for k in ["-32010", "-32011"])
        check_result("xrp.json has expected error codes", has_errors, f"keys={list(spec['exceptions'].keys())[:5]}")
    except Exception as e:
        check_result("xrp.json", False, str(e))
    ...
    try:
        with open(explorer_path) as f:  # noqa: ASYNC230
            explorers = json.load(f)
        check_result("explorers.json has XRP", "xrp" in explorers)
        if "xrp" in explorers:
            xrp_exp = explorers["xrp"]
            check_result("XRP mainnet explorer URL", "mainnet" in xrp_exp and "{}" in xrp_exp["mainnet"])
            check_result("XRP testnet explorer URL", "testnet" in xrp_exp and "{}" in xrp_exp["testnet"])
    except Exception as e:
        check_result("explorers.json", False, str(e))
        ...
        try:
            ...
            if txes:
                ...
                if tx_hash:
                    ...
                    check_result(
                        "get_confirmations (live tx)", isinstance(confs, int) and confs >= 0, f"confirmations={confs}"
                    )
                else:
                    check_result("get_transaction (live tx)", True, "skipped — no tx hash")
            else:
                check_result("get_transaction (live tx)", True, "skipped — empty ledger")
        except Exception as e:
            check_result("get_transaction (live tx)", False, str(e))
    ...
    check_result("XRPDaemon.name", XRPDaemon.name == "XRP")
    check_result("XRPDaemon.DEFAULT_PORT", XRPDaemon.DEFAULT_PORT == 5012)
    check_result("XRPDaemon.DIVISIBILITY", XRPDaemon.DIVISIBILITY == 6)
    check_result("XRPDaemon.BLOCK_TIME", XRPDaemon.BLOCK_TIME == 4)
    check_result("XRPDaemon.KEYSTORE_CLASS", XRPDaemon.KEYSTORE_CLASS is KeyStore)
    check_result("XRPDaemon.WALLET_CLASS", XRPDaemon.WALLET_CLASS is Wallet)
    check_result("XRPDaemon.INVOICE_CLASS", XRPDaemon.INVOICE_CLASS is Invoice)
    check_result("XRPDaemon.UNIT", XRPDaemon.UNIT == "drop")
    ...
    try:
        ...
        check_result("SDK: XRP importable", True)
        check_result("SDK: XRP in COINS", "XRP" in COINS)

        xrp_sdk = XRPCoin()
        check_result("SDK: coin_name", xrp_sdk.coin_name == "XRP")
        check_result("SDK: friendly_name", xrp_sdk.friendly_name == "XRP")
        check_result("SDK: RPC_URL", xrp_sdk.RPC_URL == "http://localhost:5012")
        check_result("SDK: is_eth_based", xrp_sdk.is_eth_based is True)
    except Exception:
        check_result("SDK integration", False, traceback.format_exc())
